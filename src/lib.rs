...
const NOT_IN_WORKSPACE: &str =
    "The crate `workspace-root-patch` is being used as a regular Cargo dependency.\n\
    Because it is not located within your workspace, it cannot determine the path to the workspace root.\n\
    Please add `workspace-root-patch` as a member of your workspace and patch your dependencies to use the local path.\n\n\
...
/// i.e. patched in via `cargo workspace-root-patch install`. As a regular
